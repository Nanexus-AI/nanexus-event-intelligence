...
class ReviewItem(IdMixin, CreatedAtMixin, Base):
    ...
    labels: Mapped[list[str]] = mapped_column(JSON, default=list, nullable=False)
    zones: Mapped[list[str]] = mapped_column(JSON, default=list, nullable=False)
    source_revision: Mapped[str] = mapped_column(String(128), default="unknown", nullable=False)
...
class ModelInvocation(IdMixin, CreatedAtMixin, Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("processor_job_id", name="uq_model_invocation_processor_job"),
    )

    processor_job_id: Mapped[UUID | None] = mapped_column(
        Uuid, ForeignKey("processor_jobs.id", ondelete="SET NULL")
    )
    ...
    external_network_used: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)


class ProcessorJob(IdMixin, CreatedAtMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("attempt_count >= 0", name="ck_processor_jobs_attempt_count"),
        UniqueConstraint("idempotency_key", name="uq_processor_jobs_idempotency_key"),
        Index("ix_processor_jobs_status_available", "status", "available_at"),
    )
    ...
    subject_revision: Mapped[str] = mapped_column(String(255), nullable=False)
...
class Claim(IdMixin, CreatedAtMixin, Base):
    ...
    review_item_id: Mapped[UUID | None] = mapped_column(
        Uuid, ForeignKey("review_items.id", ondelete="SET NULL")
    )
